...
{
	info("checkpoint/multicheckpoint init");
...
{
	info("checkpoint/multicheckpoint fini");
	return SLURM_SUCCESS;
}
...
extern int slurm_ckpt_comp ( struct step_record * step_ptr, time_t event_time,
		uint32_t error_code, char *error_msg )
{
	error("checkpoint/multicheckpoint: slurm_ckpt_comp not implemented");
...
extern int slurm_ckpt_task_comp ( struct step_record * step_ptr,
				  ...
				  uint32_t error_code, char *error_msg )
{
	error("checkpoint/multicheckpoint: slurm_ckpt_task_comp not implemented");
	return ESLURM_NOT_SUPPORTED;
}
			       ...
			       uint16_t protocol_version)
{
	...
	if (protocol_version >= SLURM_MIN_PROTOCOL_VERSION) {
		...
		size = 0;
		pack16(CHECK_MULTICHECKPOINT, buffer);
...
extern int slurm_ckpt_signal_tasks(stepd_step_rec_t *job, char *image_dir)
{
	...
	for (i = 0; i < job->node_tasks; i ++) {
		...
		if (children[i] < 0) {
			...
			if (setgid(job->gid) < 0) {
				error ("checkpoint/multicheckpoint: "
...
static void *_ckpt_agent_thr(void *arg)
{
	...
	debug3("checkpoint/multicheckpoint: sending checkpoint tasks request %u to %u.%u",
	...
	if (rc != SLURM_SUCCESS) {
		error("checkpoint/multicheckpoint: error on checkpoint request %u to "
