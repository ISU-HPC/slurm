/*****************************************************************************\
 *  checkpoint_criu.c - CRIU slurm checkpoint plugin.
 *  $Id: checkpoint_criu.c 0001 2008-12-29 16:50:11Z hjcao $
 *****************************************************************************
...
const char plugin_name[]       	= "CRIU checkpoint plugin";
const char plugin_type[]       	= "checkpoint/criu";
			  ...
			  uint32_t *error_code, char **error_msg )
{
	...
	case CHECK_CREATE:
		...
		if (job_ptr->cpu_cnt > 1) { //at this moment CRIU does not provide suport for parallel jobs
			rc = ESLURM_NOT_SUPPORTED;
			break;
		}
...
extern int slurm_ckpt_comp ( struct step_record * step_ptr, time_t event_time,
		uint32_t error_code, char *error_msg )
{
	error("checkpoint/criu: slurm_ckpt_comp not implemented");
...
extern int slurm_ckpt_task_comp ( struct step_record * step_ptr,
				  ...
				  uint32_t error_code, char *error_msg )
{
	error("checkpoint/criu: slurm_ckpt_task_comp not implemented");
	return ESLURM_NOT_SUPPORTED;
}
...
extern int slurm_ckpt_signal_tasks(stepd_step_rec_t *job, char *image_dir)
{
	...
	debug3("checkpoint/criu: slurm_ckpt_signal_tasks: image_dir=%s",
	       image_dir);
	/*
...
static void *_ckpt_agent_thr(void *arg)
{
	...
	debug3("checkpoint/criu: sending checkpoint tasks request %u to %u.%u",
	...
	if (rc != SLURM_SUCCESS) {
		error("checkpoint/criu: error on checkpoint request %u to "
